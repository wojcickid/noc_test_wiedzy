...
def test_zly_token_pokazuje_blad_i_nie_wpuszcza_do_testu(klient, test_z_pytaniami):
    ...
    assert "Nieprawidłowy lub już wykorzystany token dostępu.".encode("utf-8") in odpowiedz.data
...
def test_start_testu_pokazuje_pierwsze_pytanie(klient, test_z_pytaniami):
    ...
    assert b"Pytanie 1 z 3" in start.data
...
def test_pelny_przeplyw_od_startu_do_wyniku(klient, test_z_pytaniami):
    ...
    assert b"Poprawne odpowiedzi: 3" in wynik.data
    assert b"Wszystkie pytania: 3" in wynik.data
...
def test_powtorne_uzycie_tokenu_jest_zablokowane(klient, test_z_pytaniami):
    ...
    assert "Nieprawidłowy lub już wykorzystany token dostępu.".encode("utf-8") in ponownie.data
