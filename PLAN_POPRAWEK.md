...
| 5 Eksporty i raporty | zaakceptowane, zacommitowane (`012e382`) | `etap-5-eksporty-i-raporty` | 2026-09-23 | E1/UI10, UI9, E2, E3, S7 — patrz sekcja 0.5 |
| 6 Maile | zaakceptowane (ręczne testy usera na Gmailu OK) | `etap-6-maile` | 2026-09-23 | L6, F3, E6, korespondencja seryjna, wynik jednej osoby, token z linku — patrz sekcja 0.5 |
...
- **Odpowiedzi na pytania z sekcji 0.3:** zapytania do IT nie wysłano — założenie: będzie skrzynka, testy na koncie Gmail usera (później ewentualnie Google Workspace; w firmie są też skrzynki Microsoft), więc SMTP jest ogólny, a instrukcja w README celuje w Gmaila (`smtp.gmail.com:587` STARTTLS + hasło do aplikacji). Ustawienia serwera z hasłem w `config.json` (gitignored, wzór `config.przyklad.json`), panel pokazuje je bez hasła i ma przycisk maila testowego. Nazwa nadawcy (domyślnie „Test wiedzy”) i adres aplikacji do linków (domyślnie `http://localhost:5555`, z ostrzeżeniem) edytowalne w panelu, w nowej tabeli `ustawienia_aplikacji`. Szablony: prosty tekst ze stopką i „Powodzenia!”, zaproponowane przez Claude Code, edytowalne per test. Wysyłka w tle, 1 mail/s, strona postępu, status maila przy tokenie. Przypomnienia (E6) — tak. Lista uczestników: pole tekstowe (`Imię Nazwisko;email`) **i** plik XLSX/CSV. Word: XLSX + instrukcja (bez generowania `.docx`). Na życzenie usera dodano: eksport wyniku jednej osoby do XLSX ze strony tokenu.
- **L6 — migracja `przypisany` → `imie`/`email`:** jednorazowa w `_migruj_tabele` (tylko gdy brak kolumny `imie`), słowo z `@` trafia do `email`, reszta do `imie`; wartość, której nie da się rozdzielić, idzie w całości do `imie`. Stara kolumna `przypisany` **zostaje w starych bazach** (nieużywana) — usuwanie kolumny z żywej bazy to zbędne ryzyko, a nie przeszkadza. Kopia przed zmianą: `baza.db.bak-2026-09-23-etap6`.
- **Odstępstwo od zasady „migracja najpierw na kopii”:** żywa `baza.db` została zmigrowana przypadkiem, przy sprawdzaniu importu modułu `test_wiedzy_app` (import woła `inicjalizuj()`). Sprawdzone od razu: 20 tokenów, 9 z przypisaniem — wszystkie 9 mają `imie` = dawne `przypisany`, `email` NULL (żadne nie zawierało `@`); liczności testów/podejść/odpowiedzi bez zmian (3/17/67), `integrity_check` = ok. Kopia sprzed zmiany istnieje. Migracja jest dodatkowo pokryta testem na bazie w starym schemacie (`test_migracja_pola_przypisany`, także idempotentność).
- **Statusy maili:** „wysłano” = serwer nadawcy przyjął wiadomość. Status „błąd” dostają tylko błędy natychmiastowe (odrzucony adres, zły login, zerwane połączenie); zwrotki przychodzące później trafiają na skrzynkę nadawcy i aplikacja ich nie widzi — opisane w README i na stronie wysyłki. Zmiana adresu przy tokenie zeruje status maila. Zaproszenia idą tylko do tokenów bez wysłanego zaproszenia (także po błędzie), przypomnienia — do wszystkich nieukończonych; ponowne zaproszenie do wybranych przez zaznaczenie na liście tokenów.
- **Wysyłka w tle:** zwykły wątek (bez nowej zależności), naraz tylko jedna seria (dwie równoległe z jednego konta = ryzyko blokady przez Gmaila). Stan postępu jest w pamięci procesu — po restarcie serwera strona postępu znika, ale wynik każdego maila jest już w bazie. Serwer zatrzymany w trakcie serii: niewysłane tokeny zostają bez statusu i pójdą przy kolejnych „zaproszeniach”.
- **Bezpieczeństwo:** hasło SMTP nie trafia do panelu, komunikatów ani bazy (testy sprawdzają to przy złym haśle). Temat maila składany do jednej linii (bez wstrzykiwania nagłówków). Nieznane znaczniki w szablonie są odrzucane przy zapisie. Token z linku (`/?token=…`) tylko wypełnia pole — po wyczyszczeniu do liter/cyfr (max 20); token zużywa się dopiero po „Rozpoczynam” (jak od Etapu 4).
- **Naprawione znalezisko z Etapu 5:** mianownik w `admin_token_szczegoly` liczony z `podejscia.liczba_pytan` (przy „czas minął” np. 1/3 zamiast 1/1); strona domyka też przeterminowane podejścia przed pokazaniem statusu.
- **CLI `generuj_tokeny.py --lista`:** format `Imię;email` jak w panelu, przyjmuje też `.xlsx`/`.csv`; błędy w liście przerywają bez generowania.
- **Znalezione po drodze (niepoprawione, poza zakresem):** `session["nowe_tokeny"]` (lista nowo wygenerowanych tokenów po PRG, B10) siedzi w ciasteczku sesji — przy kilkuset osobach z adresami przekroczy limit ~4 KB i przeglądarka odrzuci ciasteczko, więc ramka „Nowo wygenerowane tokeny” się nie pokaże (tokeny i tak są na liście niżej, a logowanie admina zostaje, bo przeglądarka zachowuje poprzednie ciasteczko). Docelowo: trzymać w sesji tylko listę tokenów albo znacznik czasu i doczytywać z bazy.
- **Nie sprawdzone:** Microsoft 365 (`smtp.office365.com`) — tylko opis w README; SMTP AUTH bywa tam wyłączony przez administratora.
- **Testy:** 156/156 przechodzi (120 z wcześniejszych etapów — kilka zaktualizowanych pod kolumny imie/email w eksportach i formularzu przypisania — + 36 nowych w `tests/test_etap6_maile.py`, z atrapą serwera SMTP zamiast prawdziwej wysyłki).
- **Ręczna weryfikacja usera (2026-09-24, konto Gmail):** mail testowy i ukrycie hasła w panelu, adres aplikacji w sieci, generowanie tokenów z listy/pliku i wysyłka zaproszeń (postęp, status przy tokenie), link z maila (token wpisany, niezużyty przed „Rozpoczynam”), własny szablon z podglądem i przypomnienie, złe hasło (czytelny komunikat bez hasła), XLSX do korespondencji seryjnej, wynik jednej osoby w Excelu — wszystko OK. User zaakceptował etap 6.
